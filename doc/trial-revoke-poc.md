...
Use a dedicated Revoker Group that is configured as an admin group. The Revoke button calls the existing unsubscribe route.

- User must belong to the revoker group
- User must be an admin
- User must be allowed to manage the selected API group
- The revoker group must be configured as an admin group

The button is hidden unless these rules are met. The existing Portal API authorizes the unsubscribe action because admins are allowed to delete subscriptions.
...
Define the same group in the external wicked-config `groups.json` configuration with `adminGroup: true`, for example:

```json
{
	"id": "revoke",
	"name": "Revoker dev-portal Subscriptions User Group",
	"adminGroup": true,
	"approverGroup": false
}
```

Assign the group ID to each intended revoker in the user's stored `groups` array. If the identity provider sends an external group such as an ADFS group, map it through `alt_ids`, but configure `revokerGroup` with the Wicked group `id`.
...
- The route calls the existing Portal API deletion endpoint: `DELETE /applications/:appId/subscriptions/:apiId`
...
- Revoker users must have admin rights, so this is not least-privilege authorization
- The revoker-group and API-group checks currently control UI visibility; the underlying delete endpoint still uses the existing Portal API permissions

### Product Manager Explanation

Choose this when speed and low implementation effort are more important than limiting permissions. The Revoker Group is treated as an admin group, so these users can revoke a trial in one click but also have broader admin capabilities. This is the implemented POC option.
